...
function ImgGen({ prompt, setPrompt, negPrompt, setNegPrompt, resolution, setResolution, generating, setGenerating, generatedImages, setGeneratedImages, selectedCharacter, scenes, onSave }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (t) fp = t.prefix + fp;
      if (selectedCharacter) fp += `, consistent character "${selectedCharacter.name}", ${selectedCharacter.mode === "face" ? "same face, same facial features" : "same body, same appearance"}`;
      fp += ", masterpiece, best quality, highly detailed, 8K";
      const [w, h] = resolution.split("x").map(Number);
      const seed = Math.floor(Math.random() * 2147483647);

      // Check if we should use IP-Adapter (character with image selected)
      const useIPAdapter = selectedCharacter?.image;
      let wf;

      if (useIPAdapter) {
        // ── IP-Adapter Face workflow: injects character face into generation ──
        const charBase64 = selectedCharacter.image.startsWith("data:")
          ? selectedCharacter.image.split(",")[1]
          : selectedCharacter.image;
        const faceWeight = selectedCharacter.mode === "face" ? 0.85 : 0.6;

        wf = {
          // Load checkpoint
          "5": { class_type: "CheckpointLoaderSimple", inputs: { ckpt_name: "flux1-dev-fp8.safetensors" } },
          // Load IP-Adapter model
          "20": { class_type: "IPAdapterUnifiedLoader", inputs: { preset: "PLUS FACE (portraits)", model: ["5", 0] } },
          // Load character image from base64 via API input
          "21": { class_type: "LoadImage", inputs: { image: "character_ref.png" } },
          // Prepare image for CLIP Vision
          "22": { class_type: "PrepImageForClipVision", inputs: { image: ["21", 0], interpolation: "LANCZOS", crop_position: "center", sharpening: 0.0 } },
          // Apply IP-Adapter
          "23": { class_type: "IPAdapterApply", inputs: {
            ipadapter: ["20", 1],
            clip_vision: ["20", 2],
            image: ["22", 0],
            model: ["20", 0],
          ...
          // KSampler uses IP-Adapter enhanced model
          "13": { class_type: "KSampler", inputs: { seed, steps, cfg, sampler_name: "euler", scheduler: "simple", denoise: 1.0, model: ["23", 0], positive: ["6", 0], negative: ["7", 0], latent_image: ["14", 0] } },
          "8": { class_type: "VAEDecode", inputs: { samples: ["13", 0], vae: ["5", 2] } },
          "9": { class_type: "SaveImage", inputs: { filename_prefix: "ai_studio_face", images: ["8", 0] } },
        };
      } else {
        // ── Standard FLUX workflow (no character) ──
        wf = {
          "5": { class_type: "CheckpointLoaderSimple", inputs: { ckpt_name: "flux1-dev-fp8.safetensors" } },
          "14": { class_type: "EmptyLatentImage", inputs: { width: w, height: h, batch_size: 1 } },
          "6": { class_type: "CLIPTextEncode", inputs: { text: fp, clip: ["5", 1] } },
      ...
      // Build request body — include character image if using IP-Adapter
      const requestBody = { input: { workflow: wf } };
      if (useIPAdapter) {
        const charBase64 = selectedCharacter.image.startsWith("data:")
          ? selectedCharacter.image.split(",")[1]
          : selectedCharacter.image;
        requestBody.input.images = [{ name: "character_ref.png", image: charBase64 }];
      }
